...
define gitlab_ci_runner::runner (
  Hash $config,
  Optional[Boolean]          $ssl_insecure = false,
  ...
  if $_config['registration-token'] {
    ...
    $deferred_call = Deferred('gitlab_ci_runner::register_to_file', [$_config['url'], $_config['registration-token'], $_config['name'], $register_additional_options, $http_proxy, $ca_file, $ssl_insecure])
  ...
  if $ensure == 'present' {
    ...
    $absent_content = Deferred('gitlab_ci_runner::unregister_from_file', [$_config['url'], $_config['name'], $http_proxy, $ca_file, $ssl_insecure])
